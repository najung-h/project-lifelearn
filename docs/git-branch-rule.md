...
- **`main`**: 배포용, 병합 시 자동 배포(CI/CD).
- **`dev`**: 코드 통합용 브랜치, 기능 병합 후 자동화 테스트(CI) 실행.
- **`feat-`**: 기능 단위 작업 브랜치, 완료 후 PR로 `dev`에 병합
...
Github Flow를 채택해서 Fork하는 형태가  아니라, 공통 리포지토리에서 feat 브랜치를 운용하는 전략을 선택한 이유는, 2명의 소규모 팀에서 **불필요한 복잡성을 줄이고 효율성을 극대화하기 위함**입니다.
